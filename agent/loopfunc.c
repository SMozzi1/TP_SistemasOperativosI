...
void message_from_erlang(int fd, int epollfd){
    ...
    // Re-arm the one-shot fd for the next message.
    restart_epolloneshot(fd);
...
void recive_message_from_remote(int fd, int epollfd){
    ...
    /*
        After EPOLLONESHOT fires, the kernel disables monitoring for this fd, so
        we re-arm it. Skip only result == -1 (dead). restart_epolloneshot()
        tolerates ENOENT, which happens when a GRANTED just turned this fd into a
        held provider connection removed from epoll (see B1).
    */
    if (result >= 0) {
        restart_epolloneshot(fd);
    }
